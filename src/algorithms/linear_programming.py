...
class LinearProgrammingOptimizer(OptimizationAlgorithm):
    ...
    def _create_variables(self):
        ...
        for task in self.tasks:
            ...
            # 開始時刻は0-23の範囲
            self.start_time_vars[task.task_id] = pulp.LpVariable(
                var_name, lowBound=0, upBound=23, cat="Continuous"
            )
        ...
        for operator in self.operators:
            for task in self.tasks:
                if not self.can_assign(operator.operator_id, task.task_id):
                    self.problem += (
                        self.assignment_vars[(operator.operator_id, task.task_id)] == 0,
        ...
        for operator in self.operators:
            for task in self.tasks:
                ...
                self.problem += (
                    ...
                    >= work_start
                    - (1 - self.assignment_vars[(operator.operator_id, task.task_id)])
                    * 24,
                ...
                self.problem += (
                    self.start_time_vars[task.task_id] + task.required_hours
                    <= work_end
                    + (1 - self.assignment_vars[(operator.operator_id, task.task_id)])
                    * 24,
            ...
            for i, task1 in enumerate(self.tasks):
                for j, task2 in enumerate(self.tasks):
                    if i < j:  # 各ペアを一度だけチェック
                        ...
                        both_assigned = (
                            self.assignment_vars[(operator.operator_id, task1.task_id)]
                            + self.assignment_vars[
                                (operator.operator_id, task2.task_id)
                            ]
                        )
                        ...
                        self.problem += (
                            self.start_time_vars[task1.task_id] + task1.required_hours
                            <= self.start_time_vars[task2.task_id]
                            + M * (1 - order_var)
                            + M * (2 - both_assigned),
                        ...
                        self.problem += (
                            self.start_time_vars[task2.task_id] + task2.required_hours
                            <= self.start_time_vars[task1.task_id]
                            + M * order_var
                            + M * (2 - both_assigned),
        ...
        for operator in self.operators:
            for task in self.tasks:
                ...
                if (
                    ...
                    # 開始時刻を取得
                    start_hour = int(
                        round(pulp.value(self.start_time_vars[task.task_id]))
                    )
    ...
    def optimize(self, operators=None, tasks=None, **kwargs) -> ScheduleResult:
        """公開用の最適化メソッド"""
        if operators is not None and tasks is not None:
            self.setup(operators, tasks)
        return self.run()
